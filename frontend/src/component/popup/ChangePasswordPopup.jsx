...
const ChangePasswordPopup = ({ isOpen, setIsOpen }) => {
    ...
    const clickHandler = async () => {
        const result = await usePostFetch(`${import.meta.env.VITE_HOSTNAME}/api/forgotpassword`)
    ...
    const resetPasswordHandler=async()=>{
        const result = await usePostFetch(`${import.meta.env.VITE_HOSTNAME}/api/user/resetpassword`, { resetPassword })
        if(!result.success){
        ...
        <>
            <Popup open={isOpen} onClose={() => setIsOpen(false)} {...{ overlayStyle }} >
                <div className='w-[400px] border-2 border-neutral-500 shadow-md p-3 bg-white rounded-md relative'>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <p onClick={() => { setOnPassword(true), setIsOpen(false) }} className='text-blue-500 cursor-pointer my-3 font-bold'>Forgot Password</p>
            ...
            <Popup open={onPassword} onClose={() => { setOnPassword(false), setShow(false) }} {...{ overlayStyle }}>
                <div className='w-[400px] flex flex-col items-center border-2 border-neutral-500 shadow-md p-3 bg-white rounded-md '>
                    {!show && <h1 className='text-xl text-center'>Click on <span className='text-red-500 font-bold'>Get OTP</span> button and you will receive a otp on your verified email.This otp is valid for 2 min</h1>}
